...
func TestDHCPSettingsAndLeases(t *testing.T) {
	...
	loc := c.post(t, "/dhcp/settings", url.Values{
		"enabled":       {"on"},
	...
	if loc = c.post(t, "/dhcp/leases/aa:bb:cc:dd:ee:ff", lease); loc.Query().Get("err") != "" {
		t.Fatalf("lease update: %s", loc.Query().Get("err"))
	}
	got := store.Get().DHCP.StaticLeases
	if len(got) != 1 || got[0].Hostname != "scanner" || got[0].IP != "192.168.1.11" {
		t.Fatalf("lease after update: %+v", got)
	}

	if loc = c.post(t, "/dhcp/leases/aa:bb:cc:dd:ee:ff/delete", url.Values{}); loc.Query().Get("err") != "" {
		t.Fatalf("lease delete: %s", loc.Query().Get("err"))
	}
	if len(store.Get().DHCP.StaticLeases) != 0 {
